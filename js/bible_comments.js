      ...
      if (o.rez == 1) {
        ...
        while (i < comemnts.length) {
          ...
          var html =
            ...
            '<div class="row">' +
            '<div class="col-10">' +
            '<div type="button" class="comment-verse" onclick="showDescription(' +
            coment_id +
            ')">' +
            coment_title +
            "</div>" +
            ...
            ao_class() +
            '">' +
            '<a type="button" style="color: #D3D3D3;" onclick="open_edit_comment(' +
            coment_id +
            ')">Labot</a>' +
            "</div>" +
            "</div>" +
            '<div class="dictionary-description" id="description_' +
            coment_id +
            '">' +
            coment_html +
            "</div>" +
    ...
    success: function (result) {
      ...
      if (o.rez == 1) {
        ...
        while (i < list.length) {
          ...
          var html =
            '<div class="vertical-hr accordion accordion-flush" style="--bs-accordion-bg: rgba(255, 255, 255, 1);">' +
            '<div class="accordion-item">' +
            '<div class="accordion-header">' +
            '<div class="row">' +
            '<div class="col-10">' +
            '<div type="button" class="dictionary-word" onclick="showDescription(\'dictionary_' +
            dictionary_id +
            "')\">" +
            dictionary_word +
            "</div>" +
            ...
            ao_class() +
            '">' +
            '<a type="button" style="color: #D3D3D3;" onclick="open_edit_dictionary(' +
            dictionary_id +
            ')">Labot</a>' +
            "</div>" +
            "</div>" +
            '<div class="dictionary-description" id="description_dictionary_' +
            dictionary_id +
            '">' +
            dictionary_html +
            "</div>" +
...
function load_comment_books(tr) {
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      coment_names = o;
      html =
        '<li><a class="dropdown-item" href="#" onclick="changeDropdownText(\'Jaunākie\'); load_bible_comments(0);">Jaunākie</a></li>';
      if (o.length > 0) {
        var i = 0;
        while (i < o.length) {
          var id = o[i].i;
          ...
          if (count > 0) {
            html =
              html +
              '<li><a class="dropdown-item" href="#" onclick="changeDropdownText(\'' +
              a +
              cc_html +
              "'); load_bible_comments(" +
              id +
