		  ...
		  while (i < o.length) {
		   ...
		   var s = o[i].s;
			 ...
			 var c_html = '';
		  	 ...
		  	 var ic = 1;
			  while (ic <= c_max) {
		  		c_html = c_html+'<div class="accordion-body px-0 py-0"  onclick="onClickSetChapter('+b+','+ic+')" style="display: inline-block"><a type="button" class="px-4 py-4" style="display: inline-block"><h3>'+ic+'</h3></a></div>';
		  		ic++;
				} 
			book_names_html = book_names_html + '<div class="accordion accordion-flush bg-canvasbody" id="accordionChapter"><div class="accordion-item"><div class="accordion-header"><div class="row row_my-2_'+o[i].on+' " style="--bs-gutter-x: 0 !important"><a type="button" class=" text-decoration-none pb-0 collapsed" id="toggleThis"><h2 id="bookSelect" class="py-0 my-2 hr-lines" data-bs-toggle="collapse" data-bs-target="#accordionPanel_'+b+'" aria-expanded="false" aria-controls="accordionPanel_'+b+'" onclick="onclickSetBook('+ b+')">'+s+'</h2></a><div id="accordionPanel_'+b+'" class="accordion-collapse collapse px-5"">'+c_html+'</div></div></div></div></div>';
	 		i++;
		  }
	  } 
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            if (history.pushState) {
                ...
                var newTitle = getBookName(b) + ' ' + c + ' | ' + getTranslationMapping(tr) + ' | Māceklis';
            ...
            if (o.b > 0) {
                ...
                var c3 = '' + o.c;
                if (c3.length == 1) { c3 = '00' + c3; } else if (c3.length == 2) { c3 = '0' + c3; }
                var id5 = b2 + c3;
                ...
                // First verse
                chapter_html = '<div id="verse_id_1">' +
                    '<span class="verse first-verse ' + class_selected + '" data-key="' + id5 + '001">' +
                    '<h2 class="chapter-number" data-chapter="' + o.c + '"></h2>' +
                    '<span class="prose" style="font-size:1.5rem;">' +
                    '<a style="background-color: ' + verseColor + ';" onclick="handleVerseClick(\'' + id5 + '001\', ' + o.b + ', ' + o.c + ', 1)" data-bs-toggle="' + (hasComment(o.b, o.c, 1) ? 'offcanvas' : '') + '" data-bs-target="' + (hasComment(o.b, o.c, 1) ? '#sidebar_bottom' : '') + '">' +
                    '<span class="start-pericope"></span>' + o.varr[1].t + '</a>' +
                    '</span>' +
                    '</span>' +
                    '</div>';
                ...
                while (i <= o.vlen) {
                    ...
                    var verseColor = o.varr[i].color || ''; // Use color data from the server

                    chapter_html += '<div id="verse_id_' + v + '">' +
                        '<span class="verse ' + class_selected + '" data-key="' + id8 + '">' +
                        '<small data-verse="' + v + '">' +
                        '<span class="mt-1 verse">' + v + '</span>' +
                        '</small>' +
                        '<span class="prose" style="font-size:1.5rem;">' +
                        '<a style="background-color: ' + verseColor + ';" onclick="handleVerseClick(\'' + id8 + '\', ' + o.b + ', ' + o.c + ', ' + v + ')" data-bs-toggle="' + (hasComment(o.b, o.c, 1) ? 'offcanvas' : '') + '" data-bs-target="' + (hasComment(o.b, o.c, 1) ? '#sidebar_bottom' : '') + '">' + t + '</a>' +
                        '</span>' +
                        '</span>' +
                        '</div>';
                    i++;
...
function deselectVerse() {
    const oldVerseElement = document.querySelector(`[data-key="${selectedVerseId}"]`);
    if (oldVerseElement) {
...
function handleVerseClick(verseId, b, c, v) {
    ...
    if (selectedVerseId !== verseId) {
        ...
        if (hasComment(b, c, v)) {
            ...
            versePill.style.top = `${rect.top + window.scrollY - versePill.offsetHeight + 10}px`;
...
function showColorPicker(verseId) {
    selectedVerseId = verseId;
    let verseElement = document.querySelector('[data-key="' + selectedVerseId + '"]');
    ...
    if (colorPickerOpen) {
        ...
        if (verseElement) {
            ...
            colorPickerModal.style.top = `${rect.top + window.scrollY + verseElement.offsetHeight}px`;
...
function applyColor() {
    ...
    let verseElement = document.querySelector('[data-key="' + selectedVerseId + '"] .prose a');
    if (verseElement) {
...
function showCommentModal(verseId) {
    ...
    if (commentModalOpen) {
        ...
        if (verseElement) {
            ...
            commentModal.style.top = `${rect.top + window.scrollY + verseElement.offsetHeight}px`;
...
function removeComment() {
    ...
    let verseElement = document.querySelector('[data-key="' + selectedVerseId + '"] .prose');
    
    if (verseElement) {
        // Remove the class before making the AJAX call
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            commentModalOpen = false;
            document.getElementById('versePill').style.display = "none";
            deselectVerse(); // Ensure deselection happens after UI updates
        },
        error: function(xhr, status, error) {
...
function saveComment() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            let verseElement = document.querySelector('[data-key="' + verseId + '"] .prose');
            if (verseElement) {
...
function updateMetaTag(name, content) {
    var metaTag = document.querySelector('meta[name="' + name + '"]') || document.querySelector('meta[property="' + name + '"]');
    if (metaTag) {
...
// Function to update title based on URL parameters
...
function scrollFunction() {
	...
	document.getElementById("verse_id_"+jump_to_v).scrollIntoView({ behavior: "instant", duration:0, block: "center", inline: "center" });
jump_to_v = 1;
}


function loadInitURL(){
//alert(window.location.search); lv65/1.Moz./1/1
...
//$("#chapter_content").html('Notiek ielāde...');


//load_c_json('lv65','1','1');
...
var id = -1;	
	while (i < len-1 && id < 0) {
  	if (book_names[i].abb == abb) {id = i+1;} 
		i++;
	}
return id;
}
...
function updateNavigation() {
    var currentBookIndex = current_b - 1; // 0-based index of the current book
    var currentBook = book_names[currentBookIndex];
	var totalChapters = Number(currentBook.c);

    // Determine prior chapter and book
    ...
    if (current_c <= 1) {
        ...
        if (priorBookIndex <= 0) {
            priorBookIndex = book_names.length - 1; // Wrap to the last book
        } else {
            priorBookIndex--; // Move to the previous book
        }
        priorChapter = book_names[priorBookIndex].c; // Last chapter of the prior book
    } else {
        // Just move to the previous chapter in the same book
        priorChapter = current_c - 1;
    }

    // Determine next chapter and book
    ...
    if (current_c >= totalChapters) {
        ...
        if (nextBookIndex >= book_names.length - 1) {
            nextBookIndex = 0; // Wrap to the first book
        } else {
            nextBookIndex++; // Move to the next book
        }
        nextChapter = 1; // First chapter of the next book
    } else {
        // Just move to the next chapter in the same book
        nextChapter = Number(current_c) + 1; // Ensure current_c is treated as a number
    }

    // Get prior and next book details
    var priorBook = (priorBookIndex >= 0 && priorBookIndex < book_names.length) ? book_names[priorBookIndex] : {s: 'Unknown Book', c: 0};
    var nextBook = (nextBookIndex >= 0 && nextBookIndex < book_names.length) ? book_names[nextBookIndex] : {s: 'Unknown Book', c: 0};

    // Update HTML with prior, current, and next book info
    $("#priorChapter, #priorChapterBottom").html(
        '<a type="button" class="py-0 px-0 border-0" style="text-decoration: none" onclick="onClickSetChapter(' + (priorBookIndex + 1) + ',' + priorChapter + ')">' +
        '« ' + priorBook.s + ' ' + priorChapter +
        '</a>'
    );

    $("#currentChapter, #currentChapterBottom").html(
        '<a type="button" style="font-weight: 650">' + currentBook.s + ' ' + current_c + '</a>'
    );

    $("#nextChapter, #nextChapterBottom").html(
        '<a type="button" class="py-0 px-0 border-0" style="text-decoration: none" onclick="onClickSetChapter(' + (nextBookIndex + 1) + ',' + nextChapter + ')">' +
        nextBook.s + ' ' + nextChapter + ' »' +
        '</a>'
    );
}
...
function prior_c() {
    ...
    // Check if we are on the first chapter
    if (current_c <= 1) {
        // Check if we are on the first book
        if (current_b <= 1) {
            // Wrap around to the last book's last chapter
            current_b = book_names.length; // Last book
            current_c = book_names[current_b - 1].c; // Last chapter of the last book
        } else {
            // Move to the last chapter of the previous book
            current_b--;
            current_c = book_names[current_b - 1].c; // Last chapter of the previous book
        }
    } else {
        // Just move to the previous chapter
        current_c--;
    }
    load_c(); // Update the chapter content and title
...
function next_c() {
    ...
    // Check if we are on the last chapter
    if (current_c >= book.c) {
        // Check if we are on the last book
        if (current_b >= book_names.length) {
            // Wrap around to the first book's first chapter
            current_b = 1; // First book
            current_c = 1; // First chapter of the first book
        } else {
            // Move to the first chapter of the next book
            current_b++;
            current_c = 1; // First chapter of the next book
        }
    } else {
...
var url = 'php/search/bible_search.php?tr='+tr+'&s='+search;			
  ...
  $.ajax({url: url, success: function(result){
	...
	if (o.rez == 1){
		...
		//var html = o.comemnts[0].t+'dd';	  
	  
	  var comemnts = o.comemnts;
		//alert(comemnts.length);
	 var i=0;
		while (i < comemnts.length) {
			...
			var v = comemnts[i].v;
		  	...
		  	var html = '<div id="div_result_'+coment_id+'" <a type="button" onclick="results_onclick('+b+','+c+','+v+')" >'+
							//'<div>'+
            ...
            try {
                ...
                if (o.varr && o.varr.length > 0) {
                    ...
                    o.varr.forEach(function(comment) {
                        ...
                        // Add verse_has_coment class for the range of verses
                        for (var vi = v; vi <= v_to; vi++) {
                            ...
                            if ($('#verse_id_' + vi).hasClass("verse_has_coment_middle verse_has_coment_single")) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            versesWithComments.forEach(function(verseId) {
                var verseElement = document.querySelector('[data-key="' + verseId + '"]');
